'use strict';

// Copyright 2016 Google Inc.
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//    http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.


const chai = require('chai');
chai.use(require('chai-as-promised'));
chai.use(require('chai-subset'));
const expect = chai.expect;
const path = require('path');
const GenericLayout = require('../src/generic_layout');
const Presentation = require('../src/presentation');
const jsonfile = require('jsonfile');

describe('GenericLayout', function() {
    const fixturePath = path.join(path.dirname(__dirname), 'test', 'fixtures');
    const presentation = new Presentation(jsonfile.readFileSync(
        path.join(fixturePath, 'mock_presentation.json')));

    describe('with title slide', function() {
        const requests = [];

        before(function() {
            const input = {
                "objectId": "title-slide",
                "title": {
                    "rawText": "This is a title slide",
                    "textRuns": [],
                    "listMarkers": [],
                    "big": false
                },
                "subtitle": {
                    "rawText": "Your name here",
                    "textRuns": [],
                    "listMarkers": [],
                    "big": false
                },
                "backgroundImage": null,
                "bodies": [],
                "tables": [],
                "videos": [],
                "images": [],
                "notes": {
                    "rawText": "Speaker notes here.",
                    "textRuns": [],
                    "listMarkers": []
                },

            };
            const layout = new GenericLayout('', presentation, input);
            layout.appendContentRequests(requests);
        });

        it(`should insert title text`, function() {
            expect(requests).to.deep.include({
                "insertText": {
                    "text": "This is a title slide",
                    "objectId": "centered-title-element"
                }
            });
        });

        it(`should insert subtitle text`, function() {
            expect(requests).to.deep.include({
                "insertText": {
                    "text": "Your name here",
                    "objectId": "subtitle-element"
                }
            });
        });

        it(`should insert speaker notes`, function() {
            expect(requests).to.deep.include({
                "insertText": {
                    "text": "Speaker notes here.",
                    "objectId": "speaker-notes-element"
                }
            });
        });
    });

    describe('with title & body slide', function() {
        const requests = [];

        before(function() {
            const input = {
                "objectId": "body-slide",
                "title": {
                    "rawText": "Title & body slide",
                    "textRuns": [],
                    "listMarkers": [],
                    "big": false
                },
                "subtitle": null,
                "backgroundImage": null,
                "bodies": [
                    {
                        "rawText": "This is the slide body.\n",
                        "textRuns": [],
                        "listMarkers": []
                    }
                ],
                "tables": [],
                "videos": [],
                "images": []
            };
            const layout = new GenericLayout('', presentation, input);
            layout.appendContentRequests(requests);
        });

        it(`should insert title text`, function() {
            expect(requests).to.deep.include({
                "insertText": {
                    "text": "Title & body slide",
                    "objectId": "title-element"
                }
            });
        });

        it(`should insert body text`, function() {
            expect(requests).to.deep.include({
                "insertText": {
                    "text": "This is the slide body.\n",
                    "objectId": "body-element"
                }
            });
        });
    });

    describe('with two column slide', function() {
        const requests = [];

        before(function() {
            const input = {
                "objectId": "two-column-slide",
                "title": null,
                "subtitle": null,
                "backgroundImage": null,
                "bodies": [
                    {
                        "rawText": "This is the left column\n",
                        "textRuns": [],
                        "listMarkers": []
                    },
                    {
                        "rawText": "This is the right column\n",
                        "textRuns": [],
                        "listMarkers": []
                    }
                ],
                "tables": [],
                "videos": [],
                "images": []
            };
            const layout = new GenericLayout('', presentation, input);
            layout.appendContentRequests(requests);
        });

        it(`should insert left column text`, function() {
            expect(requests).to.deep.include({
                "insertText": {
                    "text": "This is the left column\n",
                    "objectId": "body-element"
                }
            });
        });

        it(`should insert right column text`, function() {
            expect(requests).to.deep.include({
                "insertText": {
                    "text": "This is the right column\n",
                    "objectId": "body-element-2"
                }
            });
        });
    });

    describe('with background images', function() {
        const requests = [];

        before(function() {
            const input = {
                "objectId": "body-slide",
                "title": null,
                "subtitle": null,
                "backgroundImage": {
                    "url": "https://placekitten.com/1600/900",
                    "width": 1600,
                    "height": 900
                },
                "bodies": [
                    {
                        "rawText": "\n",
                        "textRuns": [],
                        "listMarkers": []
                    }
                ],
                "tables": [],
                "videos": [],
                "images": []
            };
            const layout = new GenericLayout('', presentation, input);
            layout.appendContentRequests(requests);
        });

        it(`should set background image`, function() {
            expect(requests).to.deep.include({
                "updatePageProperties": {
                    "objectId": "body-slide",
                    "fields": "pageBackgroundFill.stretchedPictureFill.contentUrl",
                    "pageProperties": {
                        "pageBackgroundFill": {
                            "stretchedPictureFill": {
                                "contentUrl": "https://placekitten.com/1600/900"
                            }
                        }
                    }
                }
            });
        });
    });

    describe('with inline images', function() {
        const requests = [];

        before(function() {
            const input = {
                "objectId": "body-slide",
                "title": null,
                "subtitle": null,
                "backgroundImage": null,
                "bodies": [],
                "tables": [],
                "videos": [],
                "images": [
                    {
                        "url": "https://placekitten.com/350/315",
                        "width": 350,
                        "height": 315
                    }
                ]
            };
            const layout = new GenericLayout('', presentation, input);
            layout.appendContentRequests(requests);
        });

        it(`should create image`, function() {
            expect(requests).to.containSubset([{
                "createImage": {
                    "elementProperties": {
                        "pageObjectId": "body-slide"
                    },
                    "url": "https://placekitten.com/350/315"
                }
            }]);
        });
    });

    describe('with video', function() {
        const requests = [];

        before(function() {
            const input = {
                "objectId": "body-slide",
                "title": null,
                "subtitle": null,
                "backgroundImage": null,
                "bodies": [],
                "tables": [],
                "videos": [
                    {
                        "width": 1600,
                        "height": 900,
                        "autoPlay": true,
                        "id": "MG8KADiRbOU"
                    }
                ],
                "images": []
            };
            const layout = new GenericLayout('', presentation, input);
            layout.appendContentRequests(requests);
        });

        it(`should create video`, function() {
            expect(requests).to.containSubset([{
                "createVideo": {
                    "source": "YOUTUBE",
                    "id": "MG8KADiRbOU"
                }
            }]);
        });
    });

    describe('with table', function() {
        const requests = [];

        before(function() {
            const input = {
                "objectId": "body-slide",
                "title": null,
                "subtitle": null,
                "backgroundImage": null,
                "bodies": [],
                "tables": [
                    {
                        "rows": 5,
                        "columns": 2,
                        "cells": [
                            [
                                {
                                    "rawText": "Animal",
                                    "textRuns": [],
                                    "listMarkers": []
                                },
                                {
                                    "rawText": "Number",
                                    "textRuns": [],
                                    "listMarkers": []
                                }
                            ],
                            [
                                {
                                    "rawText": "Fish",
                                    "textRuns": [],
                                    "listMarkers": []
                                },
                                {
                                    "rawText": "142 million",
                                    "textRuns": [],
                                    "listMarkers": []
                                }
                            ],
                            [
                                {
                                    "rawText": "Cats",
                                    "textRuns": [],
                                    "listMarkers": []
                                },
                                {
                                    "rawText": "88 million",
                                    "textRuns": [],
                                    "listMarkers": []
                                }
                            ],
                            [
                                {
                                    "rawText": "Dogs",
                                    "textRuns": [],
                                    "listMarkers": []
                                },
                                {
                                    "rawText": "75 million",
                                    "textRuns": [],
                                    "listMarkers": []
                                }
                            ],
                            [
                                {
                                    "rawText": "Birds",
                                    "textRuns": [],
                                    "listMarkers": []
                                },
                                {
                                    "rawText": "16 million",
                                    "textRuns": [],
                                    "listMarkers": []
                                }
                            ]
                        ]
                    }
                ],
                "videos": [],
                "images": []
            };
            const layout = new GenericLayout('', presentation, input);
            layout.appendContentRequests(requests);
        });

        it(`should create table`, function() {
            expect(requests).to.containSubset([{
                "createTable": {
                    "elementProperties": {
                        "pageObjectId": "body-slide"
                    },
                    "rows": 5,
                    "columns": 2
                }
            }]);
        });

        it(`should create table`, function() {
            expect(requests).to.containSubset([{
                "createTable": {
                    "elementProperties": {
                        "pageObjectId": "body-slide"
                    },
                    "rows": 5,
                    "columns": 2
                }
            }]);
        });

        it(`should insert cell text`, function() {
            expect(requests).to.containSubset([{
                "insertText": {
                    "text": "Animal",
                    "cellLocation": {
                        "rowIndex": "0",
                        "columnIndex": "0"
                    }
                }
            }]);
        });
    });


    describe('with formatted text', function() {
        const requests = [];

        before(function() {
            const input = {
                "objectId": "body-slide",
                "title": null,
                "subtitle": null,
                "backgroundImage": null,
                "bodies": [
                    {
                        "rawText": "Item 1\nItem 2\n\tfoo\n\tbar\n\tbaz\nItem 3\n",
                        "textRuns": [
                            {
                                "bold": true,
                                "start": 0,
                                "end": 4
                            },
                            {
                                "italic": true,
                                "start": 7,
                                "end": 11
                            },
                            {
                                "fontFamily": "Courier New",
                                "start": 29,
                                "end": 33
                            }
                        ],
                        "listMarkers": [
                            {
                                "start": 0,
                                "end": 36,
                                "type": "unordered"
                            }
                        ]
                    }
                ],
                "tables": [],
                "videos": [],
                "images": []
            };
            const layout = new GenericLayout('', presentation, input);
            layout.appendContentRequests(requests);
        });

        it(`should apply bold style`, function() {
            expect(requests).to.containSubset([{
                "updateTextStyle": {
                    "textRange": {
                        "type": "FIXED_RANGE",
                        "startIndex": 0,
                        "endIndex": 4
                    },
                    "style": {
                        "bold": true
                    },
                    "objectId": "body-element",
                    "fields": "bold"
                }
            }]);
        });

        it(`should apply italic style`, function() {
            expect(requests).to.containSubset([{
                "updateTextStyle": {
                    "textRange": {
                        "type": "FIXED_RANGE",
                        "startIndex": 7,
                        "endIndex": 11
                    },
                    "style": {
                        "italic": true
                    },
                    "objectId": "body-element",
                    "fields": "italic"
                }
            }]);
        });

        it(`should apply font family style`, function() {
            expect(requests).to.containSubset([{
                "updateTextStyle": {
                    "textRange": {
                        "type": "FIXED_RANGE",
                        "startIndex": 29,
                        "endIndex": 33
                    },
                    "style": {
                        "fontFamily": "Courier New"
                    },
                    "objectId": "body-element",
                    "fields": "fontFamily"
                }
            }]);
        });

        it(`should create bulleted list`, function() {
            expect(requests).to.containSubset([{
                "createParagraphBullets": {
                    "textRange": {
                        "type": "FIXED_RANGE",
                        "startIndex": 0,
                        "endIndex": 36
                    },
                    "bulletPreset": "BULLET_DISC_CIRCLE_SQUARE",
                    "objectId": "body-element"
                }
            }]);
        });
    });
});
